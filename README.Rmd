...
Le jeu de données `ovocyte.rds` porte sur la maturation d'ovocytes. Afin de faire maturer les ovocytes, différentes concentrations connue d'hypoxantine sont testées. Ce tableau de `r nrow(ovo)` observations comprend `r ncol(ovo)` variables. Les concentrations suivantes en hypoxantine sont employée afin d'étudier la maturation des ovocytes : `r paste0(conc, collapse = ", ")`. La variable `mat` comprend `r nlevels(ovo$mat)` niveaux :
